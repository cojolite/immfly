...
public class FlightRestTemplateClientTest {
	...
	void testCache() {
		...
		given(restTemplate.exchange(
                ...
                HttpMethod.GET,
                null, Flight.class, tailNumber, flightNumber)).willReturn(new ResponseEntity<>(flightInfo(), HttpStatus.OK));
		//when
		Flight fligth = flightRestClient.getFlightInformation(tailNumber, flightNumber);
		//then
		assertNotNull(fligth);
	}
	...
	void testResourseNotFound() {
		...
		given(restTemplate.exchange(
                "http://external:8085/v1/flight/{tailNumber}/{flightNumber}",
                HttpMethod.GET,
                null, Flight.class, tailNumber, flightNumber)).willThrow(new HttpClientErrorException(HttpStatus.NOT_FOUND));
		//when
		Flight fligth = flightRestClient.getFlightInformation(tailNumber, flightNumber);
		//then
		assertNull(fligth);
